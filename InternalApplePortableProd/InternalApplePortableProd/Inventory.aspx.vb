...
Partial Class Inventory
    ...
    '            sql = "SELECT vwInventoryView.*, ISNULL(Total,0) as TotalInventory, ISNULL(vwInventoryView.Available, ' ') as QuantityAvailableByDate, STRING_AGG( ISNULL(tbldepartment.Description, ' '), '<br>') As deptdesc2 from vwInventoryView join tblDepartmentInventory on vwInventoryView.pkInventoryID = tblDepartmentInventory.fkInventoryID join tblDepartment on tblDepartment.pkDepartmentID = tblDepartmentInventory.fkDepartmentID WHERE isDeleted = 0 and vwInventoryView.pkInventoryID in (select distinct fkInventoryID from tblDepartmentInventory tDI inner join tblDepartment tD on tDI.fkDepartmentID = tD.pkDepartmentID where tD.fkCompanyID = " & puser.company & ") " & exhibitremoval &
    '           "and tbldepartment.pkDepartmentID  in (1006,1007,1008) Group by vwInventoryView.fkInventoryTypeID, vwInventoryView.Total, vwInventoryView.Available, vwInventoryView.pkInventoryID, vwInventoryView.Name, vwInventoryView.Description , vwInventoryView.DateModified, vwInventoryView.fkInventoryTypeID," &
    '           "vwInventoryView.Notes, vwInventoryView.UDF01, vwInventoryView.UDF02, vwInventoryView.UDF03, vwInventoryView.isDeleted, vwInventoryView.fkDepartmentID, vwInventoryView.Weight, vwInventoryView.Cost, vwInventoryView.Type,vwInventoryView.SortingSequence, vwInventoryView.deptdesc, vwinventoryview.Available, vwInventoryView.Total, vwInventoryView.PartNum , vwInventoryView.Picture, vwInventoryView.DateCreated, vwInventoryView.events, vwInventoryView.threshold,vwInventoryView.Location Order by SortingSequence asc"
    '        Else
    ...
    '    'Hanhwa 1001
    '    'Apple 1002
    '    'Amazon Business Domestic 1003
    '    'Amazon EU 1004 English
    '    'Amazon EU 1005 Italian
    '    'Amazon EU 1006 French
    '    'Amazon EU 1007 Spanish
    '    'Amazon EU 1008 German
    '    '
    '    Dim sql = String.Empty
    ...
    '    If puser.role = 1003 Then
    '        sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 ORDER BY type"
    '    Else
    '        Select Case company
    '            Case 1000
    '                sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 and fkCompanyID = '1000' ORDER BY type"
    '            Case 1001
    '                sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 and fkCompanyID = '1001' ORDER BY type"
    '            Case 1002
    '                Select Case puser.department
    '                    Case 1002
    '                        sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 and PartNum >= 5000 and PartNum < 6000  and deptdesc = 'Retail' ORDER BY Type"
    '                    Case 1003
    '                        sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 and PartNum < 2000 and deptdesc = 'Recruiting' ORDER BY Type"
    '                    Case 1005
    '                        sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 and deptdesc = 'Recruiting EU' ORDER BY Type"
    '                    Case Else
    '                        sql = "Select * from vwInventoryView where  deptdesc = 'Recruiting' and isDeleted=0 and total > 0"
    '                End Select

    '            Case 1003
    '                sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 ORDER BY type"
    '            Case 1004
    '                sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 AND fkLanguageID = 1003 ORDER BY type"
    '            Case 1005
    '                sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 AND fkLanguageID = 1000 ORDER BY type"
    '            Case 1006
    '                sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 AND fkLanguageID = 1001 ORDER BY type"
    '            Case 1007
    '                sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 AND fkLanguageID = 1002 ORDER BY type"
    '            Case 1008
    '                sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 AND fkLanguageID = 1004 ORDER BY type"
    '        End Select
    '    End If
    ...
    '    '        sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 and PartNum >= 5000 and PartNum < 6000  and deptdesc = 'Retail' ORDER BY description"
    '    '    End If

    '    '    If puser.department = 1003 Then
    '    '        sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 and PartNum < 2000 and deptdesc = 'Recruiting' ORDER BY description"
    '    '    End If
    ...
    '    '    'End If
    '    'Else
    '    '    sql = "SELECT * from vwInventoryView WHERE isDeleted = 0 and (PartNum < 2000) OR (PartNum >= 5000 and PartNum < 6000) ORDER BY description"
    '    'End If
    ...
    Protected Sub RadGrid1_ItemDataBound(ByVal sender As Object, ByVal e As GridItemEventArgs)
        ...
        'If TypeOf e.Item Is GridHeaderItem AndAlso hasDateRange Then
        '    Dim header As GridHeaderItem = CType(e.Item, GridHeaderItem)
        '    If puser.role <> 1003 Then
        '        header("Available").Visible = False
        '    End If
        'End If
    ...
    Protected Sub RadGrid1_ItemCreated(ByVal sender As Object, ByVal e As GridItemEventArgs)
        If TypeOf e.Item Is GridEditableItem AndAlso e.Item.IsInEditMode Then
            ...
            'Dim pkid = editItem.GetDataKeyValue("pkInventoryID")
            'Dim dt = ReadRecords("Select fkDepartmentID from tblDepartmentInventory where fkInventoryID=" & pkid)
            'If dt.Rows.Count > 0 Then
            '    For Each r In dt.Rows
            '        Dim combo = TryCast(editItem.FindControl("RadComboBox1"), RadComboBox)
            '        For Each item As RadComboBoxItem In combo.Items
            ...
            If Not editedItem.ParentItem Is Nothing Then
                ...
                'Dim pkid = editedItem.GetDataKeyValue("pkInventoryID")
                'Dim dt = ReadRecords("Select fkDepartmentID from tblDepartmentInventory where fkInventoryID=" & pkid)
                'If dt.Rows.Count > 0 Then
                '    For Each r In dt.Rows
                '        Dim combo = TryCast(editedItem.FindControl("RadComboBox1"), RadComboBox)
                '        For Each item As RadComboBoxItem In combo.Items
                '            If item.Value = r(0) Then
                '                item.Checked = True

                '                'Dim index As Integer = dept.FindItemIndexByValue(r(0))
                '                'dept.SelectedIndex = index

                '                combo.SelectedValue = r(0)
            ...
            'Select Case r(0)
            '    Case 1000
            '        Dim chk As CheckBox = e.Item.FindControl("CheckBox1")
            '        chk.Checked = True
            '    Case 1001
            '        Dim chk2 As CheckBox = e.Item.FindControl("CheckBox2")
            '        chk2.Checked = True
            '    Case 1002
            '        Dim chk3 As CheckBox = e.Item.FindControl("CheckBox3")
            '        chk3.Checked = True
            '    Case 1003
            '        Dim chk4 As CheckBox = e.Item.FindControl("CheckBox4")
            '        chk4.Checked = True
            '    Case 1004
            '        Dim chk5 As CheckBox = e.Item.FindControl("CheckBox5")
            '        chk5.Checked = True
            '    Case 1005
            '        Dim chk6 As CheckBox = e.Item.FindControl("CheckBox6")
            '        chk6.Checked = True
            'End Select
    ...
    Sub InsertUpdate(cmd As String, ByVal e As GridCommandEventArgs)
        ...
        Dim targetFolder As String = "F:\Production\Assets\CL1001\IMG"
        'Dim targetFolder As String = "C:\Users\JasonNguyen\OneDrive - Pinnacle Exhibits\Documents\My Web Sites\InternalAppleSite\Account\inventory\"
        ...
        Using strConnStr As New SqlConnection(ConfigurationManager.ConnectionStrings("Portables").ToString)
            ...
            Else
                sqlComm.CommandText = "Update tblInventory set [fkCompanyID]=@fkCompanyID,[PartNum]=@PartNum,[Description]=@Description" & uploadedfile & ",[UDF01]=@UDF01,[UDF02]=@UDF02,[UDF03]=@UDF03,[UDF04]=@UDF04,[UDF05]=@UDF05,[UDF06]=@UDF06, " &
                    "[UDF07]=@UDF07,[UDF08]=@UDF08,[UDF09]=@UDF09,[UDF10]=@UDF10,[DateModified]=@DateModified,[LastModifiedBy]=@LastModifiedBy, isDeleted=@isDeleted, fkInventoryTypeID=@fkInventoryTypeID, weight=@weight, notes=@notes, name=@name, threshold=@threshold, cost=@cost, fkDepartmentID = @fkDepartmentID where pkInventoryID=@pkInventoryID;Delete from tblDepartmentInventory where fkInventoryID=@pkInventoryID"
    ...
    Protected Sub RadButton3_Click(sender As Object, e As EventArgs) Handles RadButton3.Click
        ...
        Try
            ...
            If startDate < today Then
                message.Text = "Start date cannot be less than current date"
            ...
            Else
                ...
                Dim outboundBufferDays = ConfigurationSettings.AppSettings("OutBoundbufferDays")
                Dim inboundBufferDays = ConfigurationSettings.AppSettings("InBoundbufferDays")


                startDate = MinusBusinessDays(startDate, 7)
                ...
                If puser.role = 1003 Then
                    sql = "SELECT case when fkInventoryTypeID in (1003,1004,1005,1006,1007,1008,1009,1011) then dbo.GetATPConsumableByDateRangeBuffer(pkInventoryID,N'" & startDate & "', N'" & endDate & "','" & inboundBufferDays & "','" & -outboundBufferDays & "') else dbo.GetATPByDateRangeBuffer(pkInventoryID,N'" & startDate & "', N'" & endDate & "','" & inboundBufferDays & "','" & -outboundBufferDays & "') end as [QuantityAvailableByDate], vwInventoryView.*,ISNULL(Total,0) as TotalInventory, ISNULL(vwInventoryView.Available, ' ') as Available, STRING_AGG( ISNULL(tbldepartment.Description, ' '), '<br>') As deptdesc2 from vwInventoryView join tblDepartmentInventory on vwInventoryView.pkInventoryID = tblDepartmentInventory.fkInventoryID join tblDepartment on tblDepartment.pkDepartmentID = tblDepartmentInventory.fkDepartmentID WHERE isDeleted = 0" & exhibitremoval &
                "Group by vwInventoryView.fkInventoryTypeID, vwInventoryView.Total, vwInventoryView.Available, vwInventoryView.pkInventoryID, vwInventoryView.Name, vwInventoryView.Description , vwInventoryView.DateModified, vwInventoryView.fkInventoryTypeID," &
            "vwInventoryView.Notes, vwInventoryView.UDF01, vwInventoryView.UDF02, vwInventoryView.UDF03, vwInventoryView.isDeleted, vwInventoryView.fkDepartmentID, vwInventoryView.Weight, vwInventoryView.Cost, vwInventoryView.Type,vwInventoryView.SortingSequence, vwInventoryView.deptdesc, vwinventoryview.Available, vwInventoryView.Total, vwInventoryView.PartNum , vwInventoryView.Picture, vwInventoryView.DateCreated, vwInventoryView.events, vwInventoryView.threshold,  vwInventoryView.Location Order by SortingSequence asc"
                End If
                ...
                'Super user role of 1001 gets to see everything within their company, based on the item's departmentID referencing companyID against the user login companyID, logic has been added to limit part number return to EXCLUDE exhibit part numbers
                If puser.role = 1001 Then
                    If puser.userID = 1056 Or puser.userID = 1053 Or puser.userID = 1035 Then
                        'sql = "SELECT * from vwInventoryView LEFT JOIN tblDepartment on vwInventoryView.fkDepartmentID = tblDepartment.pkDepartmentID WHERE isDeleted = 0 AND tblDepartment.fkCompanyID =" & puser.company & exhibitremoval
                        sql = "SELECT case when fkInventoryTypeID in (1003,1004,1005,1006,1007,1008,1009,1011) then dbo.GetATPConsumableByDateRangeBuffer(pkInventoryID,N'" & startDate & "', N'" & endDate & "','" & inboundBufferDays & "','" & -outboundBufferDays & "') else dbo.GetATPByDateRangeBuffer(pkInventoryID,N'" & startDate & "', N'" & endDate & "','" & inboundBufferDays & "','" & -outboundBufferDays & "') end as [QuantityAvailableByDate], vwInventoryView.*, ISNULL(Total,0) as TotalInventory, ISNULL(vwInventoryView.Available, ' ') as Available, STRING_AGG( ISNULL(tbldepartment.Description, ' '), '<br>') As deptdesc2 from vwInventoryView join tblDepartmentInventory on vwInventoryView.pkInventoryID = tblDepartmentInventory.fkInventoryID join tblDepartment on tblDepartment.pkDepartmentID = tblDepartmentInventory.fkDepartmentID WHERE isDeleted = 0 and vwInventoryView.pkInventoryID in (select distinct fkInventoryID from tblDepartmentInventory tDI inner join tblDepartment tD on tDI.fkDepartmentID = tD.pkDepartmentID where tD.fkCompanyID = " & puser.company & ") " & exhibitremoval &
                "Group by vwInventoryView.fkInventoryTypeID, vwInventoryView.Total, vwInventoryView.Available, vwInventoryView.pkInventoryID, vwInventoryView.Name, vwInventoryView.Description , vwInventoryView.DateModified, vwInventoryView.fkInventoryTypeID," &
                "vwInventoryView.Notes, vwInventoryView.UDF01, vwInventoryView.UDF02, vwInventoryView.UDF03, vwInventoryView.isDeleted, vwInventoryView.fkDepartmentID, vwInventoryView.Weight, vwInventoryView.Cost, vwInventoryView.Type,vwInventoryView.SortingSequence, vwInventoryView.deptdesc, vwinventoryview.Available, vwInventoryView.Total, vwInventoryView.PartNum , vwInventoryView.Picture, vwInventoryView.DateCreated, vwInventoryView.events, vwInventoryView.threshold ,vwInventoryView.Location Order by SortingSequence asc"
                ...
                "vwInventoryView.Notes, vwInventoryView.UDF01, vwInventoryView.UDF02, vwInventoryView.UDF03, vwInventoryView.isDeleted, vwInventoryView.fkDepartmentID, vwInventoryView.Weight, vwInventoryView.Cost, vwInventoryView.Type,vwInventoryView.SortingSequence, vwInventoryView.deptdesc, vwinventoryview.Available, vwInventoryView.Total, vwInventoryView.PartNum , vwInventoryView.Picture, vwInventoryView.DateCreated, vwInventoryView.events, vwInventoryView.threshold ,vwInventoryView.Location Order by SortingSequence asc"
                    Else
                        sql = "SELECT case when fkInventoryTypeID in (1003,1004,1005,1006,1007,1008,1009,1011) then dbo.GetATPConsumableByDateRangeBuffer(pkInventoryID,N'" & startDate & "', N'" & endDate & "','" & inboundBufferDays & "','" & -outboundBufferDays & "') else dbo.GetATPByDateRangeBuffer(pkInventoryID,N'" & startDate & "', N'" & endDate & "','" & inboundBufferDays & "','" & -outboundBufferDays & "') end as [QuantityAvailableByDate], vwInventoryView.*, ISNULL(Total,0) as TotalInventory, ISNULL(vwInventoryView.Available, ' ') as Available, vwInventoryView.PartNum as PartNum, STRING_AGG( ISNULL(tbldepartment.Description, ' '), '<br>') As deptdesc2 from vwInventoryView join tblDepartmentInventory on vwInventoryView.pkInventoryID = tblDepartmentInventory.fkInventoryID join tblDepartment on tblDepartment.pkDepartmentID = tblDepartmentInventory.fkDepartmentID WHERE isDeleted = 0 and vwInventoryView.pkInventoryID in (select fkInventoryID from tblDepartmentInventory tDI inner join tblUsers tU on tDI.fkDepartmentID = tU.fkDepartmentID where tU.pkUserID = " & puser.userID & ")" & exhibitremoval &
            "Group by vwInventoryView.fkInventoryTypeID, vwInventoryView.Total, vwInventoryView.Available,  vwInventoryView.pkInventoryID, vwInventoryView.Name, vwInventoryView.Description , vwInventoryView.DateModified, vwInventoryView.fkInventoryTypeID," &
            "vwInventoryView.Notes, vwInventoryView.UDF01, vwInventoryView.UDF02, vwInventoryView.UDF03, vwInventoryView.isDeleted, vwInventoryView.fkDepartmentID, vwInventoryView.Weight, vwInventoryView.Cost, vwInventoryView.Type,vwInventoryView.SortingSequence, vwInventoryView.deptdesc, vwinventoryview.Available, vwInventoryView.Total, vwInventoryView.PartNum , vwInventoryView.Picture, vwInventoryView.DateCreated, vwInventoryView.events, vwInventoryView.threshold, vwInventoryView.Location Order by SortingSequence asc"
                    End If
                ...
                '    sql = "SELECT case when fkInventoryTypeID = 1 then dbo.GetATPConsumableByDateRangeBuffer(pkInventoryID,N'" & startDate & "', N'" & endDate & "','" & inboundBufferDays & "','" & -outboundBufferDays & "') else dbo.GetATPByDateRangeBuffer(pkInventoryID,N'" & startDate & "', N'" & endDate & "','" & inboundBufferDays & "','" & -outboundBufferDays & "') end as [QuantityAvailableByDate], ISNULL(Total,0) as TotalInventory, STRING_AGG( ISNULL(vwInventoryView.deptdesc, ' '), '<br>') As deptdesc2, * FROM vwInventoryView where isdeleted = 0" & exhibitremoval &
                '    "Group by vwInventoryView.deptdesc, vwInventoryView.Total, vwInventoryView.Available, vwInventoryView.pkInventoryID, vwInventoryView.Name, vwInventoryView.Description , vwInventoryView.DateModified, vwInventoryView.fkInventoryTypeID," &
                '"vwInventoryView.Notes, vwInventoryView.UDF01, vwInventoryView.UDF02, vwInventoryView.UDF03, vwInventoryView.isDeleted, vwInventoryView.fkDepartmentID, vwInventoryView.Weight, vwInventoryView.Cost, vwInventoryView.Type,vwInventoryView.SortingSequence, vwInventoryView.deptdesc, vwinventoryview.Available, vwInventoryView.Total, vwInventoryView.PartNum , vwInventoryView.Picture, vwInventoryView.DateCreated, vwInventoryView.events, vwInventoryView.threshold"
                ...
                If puser.role = 1003 Or puser.role = 1001 Or puser.role = 1006 Then
                    message.Text = "Additional time is required to process and return the inventory based on the fulfillment location for the selected date range."

                    bufferDatesTbl.Visible = True


                    Dim sDate As Date = tbStartDate.SelectedDate
                    Dim eDate As Date = tbPickupDate.SelectedDate

                    usaStart.Text = MinusBusinessDays(sDate, 7)
                    usaEnd.Text = AddBusinessDays(eDate, 10)

                    EuStart.Text = MinusBusinessDays(sDate, 7)
                    EuEnd.Text = AddBusinessDays(eDate, 10)

                    JapanStart.Text = MinusBusinessDays(sDate, 5)
                    JapanEnd.Text = AddBusinessDays(eDate, 8)

                    ChinaStart.Text = MinusBusinessDays(sDate, 6)
                    ChinaEnd.Text = AddBusinessDays(eDate, 9)

                    AuStart.Text = MinusBusinessDays(sDate, 8)
                    AuEnd.Text = AddBusinessDays(eDate, 11)


                    'message.Text = "Inventory availability for selected dates (" + startDate + "-" + endDate + ") includes additional time to process and return the order."
                    messageDiv.Attributes.CssStyle.Add("Color", "#333333")
                Else
                    message.Text = "Inventory availability for selected dates includes additional time to process and return the order."
                    messageDiv.Attributes.CssStyle.Add("Color", "#000000")
                End If
            End If
        Catch ex As Exception
        End Try
    ...
    Protected Sub RadGrid1_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles RadGrid1.NeedDataSource
        ...
        Dim endDate As Date = DateAdd("d", 1, startDate)

        Dim outboundBufferDays = 0
        Dim inboundBufferDays = 0
        ...
        RadGrid1.DataSource = ReadRecords(sql)
        If puser.role <> 1003 Then
            RadGrid1.MasterTableView.GetColumn("Available").Visible = False
        End If
